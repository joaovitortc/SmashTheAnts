...
function moveInsect() {
    ...
    if (bomb) insect.style.backgroundImage = "url('bomb.png')";
    else insect.style.backgroundImage = "url('regular_ant.png')";
    ...
    } else insect.removeEventListener("click", handleBombClick);
}
...
function updateTimer() {
    ...
    if (timeLeft === 0) {
        finishGame();
        const finalMessage1 = document.getElementById("finalMessage");
        finalMessage1.textContent = "Time's up";
    } else timeLeft--;
}
...
function sound() {
    ...
    let snd;
    if (random > 0.5) snd = new Audio("splash_1.mp3");
    else snd = new Audio("splash_2.mp3");
    snd.play();
}
function bombSound() {
    bombSnd.play();
...
//# sourceMappingURL=index.513cc7c1.js.map
